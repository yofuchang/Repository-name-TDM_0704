...
static int XRFClk_InitI2C(void)
{
	...
	/* i2c1
	 *
	 * In the SDT flow XIicPs_LookupConfig() expects the peripheral base
	 * address. In the classic flow it expects Device ID 1.
	 */
#ifdef SDT
#if defined(XPAR_XIICPS_1_BASEADDR)
#define RFCLK_I2C1_LOOKUP_VALUE ((UINTPTR)XPAR_XIICPS_1_BASEADDR)
#elif defined(XPAR_PSU_I2C_1_BASEADDR)
#define RFCLK_I2C1_LOOKUP_VALUE ((UINTPTR)XPAR_PSU_I2C_1_BASEADDR)
#else
	/*
	 * Zynq UltraScale+ MPSoC PS I2C1 base address. This fallback is intended
	 * for ZCU208 SDT BSPs that omit the legacy xparameters macro.
	 */
#define RFCLK_I2C1_LOOKUP_VALUE ((UINTPTR)0xFF030000U)
#endif
#else
#define RFCLK_I2C1_LOOKUP_VALUE ((UINTPTR)1U)
#endif

	PRINTF("RFCLK: I2C1 lookup value = 0x%08X\r\n",
	       (unsigned int)RFCLK_I2C1_LOOKUP_VALUE);

	Config_iic = XIicPs_LookupConfig(RFCLK_I2C1_LOOKUP_VALUE);
	if (NULL == Config_iic) {
		PRINTF("RFCLK ERROR: XIicPs_LookupConfig(I2C1) returned NULL.\r\n");
		return XST_FAILURE;
	}

	PRINTF("RFCLK: I2C1 config base = 0x%08X\r\n",
	       (unsigned int)Config_iic->BaseAddress);

	Status = XIicPs_CfgInitialize(&Iic1, Config_iic,
				      Config_iic->BaseAddress);
	if (Status != XST_SUCCESS) {
		PRINTF("RFCLK ERROR: XIicPs_CfgInitialize(I2C1) failed, status=%d.\r\n",
		       Status);
		return XST_FAILURE;
	}
	...
	if (Status != XST_SUCCESS) {
		PRINTF("RFCLK ERROR: XIicPs_SetSClk(I2C1) failed, status=%d.\r\n",
		       Status);
		return XST_FAILURE;
	}

	PRINTF("RFCLK: I2C1 initialized at %u Hz.\r\n",
	       (unsigned int)ClkRate);

#undef RFCLK_I2C1_LOOKUP_VALUE
#else
#ifdef XPS_BOARD_ZCU111
	/* i2c0 */
