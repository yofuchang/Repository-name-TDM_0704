...
typedef struct {
    ...
    XRadar_beamformer_uio_map maps[ MAX_UIO_MAPS ];
} XRadar_beamformer_uio_info;

/***************** Variable Definitions **************************************/
static XRadar_beamformer_uio_info uio_info;
...
static int uio_info_read_name(XRadar_beamformer_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XRadar_beamformer_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XRadar_beamformer_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XRadar_beamformer_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XRadar_beamformer_Initialize(XRadar_beamformer *InstancePtr, const char* InstanceName) {
	XRadar_beamformer_uio_info *InfoPtr = &uio_info;
